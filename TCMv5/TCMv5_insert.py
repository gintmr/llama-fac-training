...
data_path = "TCMv5/TCMv5_format.jsonl"
...
def count_down_RL(sub_cot, indice, TCMv5_length):
    inserted_cot = f""
...
def insert_token_RL(data_path):
    inserted_data_path = data_path.replace(".jsonl", "_TCMv5.jsonl")
    ...
    with open(data_path, "r") as f:
        ...
        for data in tqdm(datas, desc="inserting token with RL format"):
            ...
            if length_of_tokenized_cot > 100050:
                ...
                sub_cot, indice, TCMv5_length = split_array_by_bins(tokenized_cot, bins)
                inserted_cot = count_down_RL(sub_cot, indice, TCMv5_length)  
                response = inserted_cot + answer

                # add_prompt = f'\n(Respond in {TCMv5_length} tokens or fewer. Complete the process between <think> and </think> within the token budget. Display the countdown exponentially as <remaining>xxx</remaining>, where xxx = 50 * 2^n, n >= 0. Think more concisely as countdown decreases.)\n'
                # add_response = f"\n(I will complete the process within {TCMv5_length} tokens and show the countdown as <remaining>xxx</remaining>, following the exponential rule.I will think more concisely as countdown decreases.)\n"
                
                # add_prompt = f"\n(Complete thinking within {TCMv5_length} tokens or fewer.)\n"
                add_prompt = f"\n(Complete thinking within \n<remaining>{TCMv5_length}</remaining>\n tokens or fewer.)"
                add_response = ""
